 ...
 * يحتوي على جميع بيانات المساقات
 * @constant {Object}
 */
const courses = {
    biology: { 
...
/**
 * الحصول على مساقات فصل معين
 * @param {number} sem - رقم الفصل (1 أو 2)
 * @returns {Array} مصفوفة بمفاتيح المساقات
 */
function getCoursesBySemester(sem) {
    return Object.keys(courses).filter(key => courses[key].semester === sem);
}

/**
 * الحصول على بيانات مساق معين
 * @param {string} key - مفتاح المساق
 * @returns {Object|null} بيانات المساق أو null
 */
 ...
 * @returns {Array} مصفوفة بالمفاتيح المطابقة
 */
function searchCourses(term) {
    term = term.toLowerCase().trim();
    if (!term) return [];
    
    return Object.keys(courses).filter(key => 
        courses[key].title.toLowerCase().includes(term) ||
        courses[key].code.toLowerCase().includes(term)
    );
}
